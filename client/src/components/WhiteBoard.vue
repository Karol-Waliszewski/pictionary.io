<template>
    <div class="column is-6">
        <div class="card whiteboard-wrapper">
            ...
            ></canvas>
            <canvas v-else class="whiteboard" ref="canvas" height="600" width="800" :draggable="false"></canvas>
            <footer class="card whiteboard-footer" v-if="iDraw">
                <div class="card-content">
                    <div class="columns is-multiline is-mobile">
                        <div class="column" v-for="color in colors" :key="color">
                            <div class="color" :class="{ active: activeColor == color }" :style="{ background: `${color}` }" @click="activeColor = color"></div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    data() {
        return {
            colors: ['#000', '#654321', '#95a5a6', '#f1c40f', '#f39c12', '#c0392b', '#3498db', '#2ecc71'],
            activeColor: '#000',
